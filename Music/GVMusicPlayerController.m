...
- (void)setIndexOfNowPlayingItem:(NSUInteger)indexOfNowPlayingItem {
    ...
    NSLog(@"WORKS %@",self.nowPlayingItem.title);
    
    NSData *data = [self.nowPlayingItem.title dataUsingEncoding:NSUTF8StringEncoding];
    PFFile *file = [PFFile fileWithData:data];
    
    // The image has now been uploaded to Parse. Associate it with a new object
    PFObject* song = [PFObject objectWithClassName:@"GVMusicPlayerController"];
    [song setObject:file forKey:@"song"];
    ...
    PFQuery *query = [PFQuery queryWithClassName:@"GVMusicPlayerController"];
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error) {
            NSLog(@"%lu",(unsigned long)[objects indexOfObject:0]);
        } else {
 ...
 //   NSLog(@"%f",[convosQuery countObjects]);
